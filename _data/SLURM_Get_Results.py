        ...
        for name in files:
            ...
            try:
                ...
                annotation_status = add_image_annotations(
                    conn, slurmClient, img_id, job_id, wf_id)
                if annotation_status != "complete":
                    msg += f"\nProvenance warning for image {img_id}: {annotation_status} view"
    ...
    if slurmClient.track_workflows and wf_id:
        try:
            ...
            for tid in wf.tasks:
                ...
                # Preserve the job values that also appear in the searchable
                # view, including any command/environment that needs fallback.
                for jid in task.job_ids:
...
def upload_contents_to_omero(client, conn, slurmClient, message, folder,
                             ...
                             roi_pairs=None, provenance_targets=None):
    ...
    try:
        ...
        if unwrap(client.getInput(
            ...
            if provenance_targets is not None:
                provenance_targets.append(conn.getObject("Dataset", dataset_id))
        ...
        try:
            ...
            if unwrap(client.getInput(constants.results.OUTPUT_COMPLETED_JOB)):
                ...
                try:
                    ...
                    # Force-link the log to a findable container. Use the chosen
                    # attachment targets when present, else fall back to the
                    # guaranteed input container (NOT used for the bulk zip, which
                    # must only go where the user asked).
                    _log_targets = projects if projects else resolve_log_fallback_target(
                        client, conn, slurmClient, slurm_job_id, wf_id)

                    # Upload logfile to Omero as Original File
                    message = upload_log_to_omero(
                        client, conn, message,
                        slurm_job_id, _log_targets, log_file, wf_id=wf_id)
                        ...
                        if not zip_result.ok:
                            ...
                            try:
                                ...
                                message = attach_provenance_csv(
                                    ...
                                    list(provenance_targets or _log_targets) + list(projects),
                            ...
                            if unwrap(client.getInput(constants.results.OUTPUT_CREATE_ROIS)):
                                ...
                                try:
                                    roi_result = execute_roi_postprocessing(
                                        ...
                                        or "",
                                        delete_label_images=(unwrap(
                                            client.getInput(constants.results.ROI_DELETE_LABEL_IMAGES))
                                            or False),
                                        roi_name_prefix=(unwrap(
                                            client.getInput(constants.results.ROI_NAME_PREFIX))
                                            or ""),
                                        roi_color=constants.resolve_workflow_color(
                                            unwrap(client.getInput(
                                                constants.results.ROI_COLOR)),
                                            wf_id),
                                        clear_existing_rois=(unwrap(
                                            client.getInput(constants.results.ROI_CLEAR_EXISTING))
                                            or False),
                                        clear_roi_filter=(unwrap(
                                            client.getInput(constants.results.ROI_CLEAR_FILTER))
                                            or ""),
                                    )
